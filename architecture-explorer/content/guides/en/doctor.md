Read-only checks that report integrity problems without repairing them.

## The problem at this stage

Read-only checks that report integrity problems without repairing them. It is one part of the larger document journey. Detection must not silently rewrite historical data.
...
Doctor checks graph lineage, migration/index state and SQLite integrity. F3 records remaining coverage issues.
