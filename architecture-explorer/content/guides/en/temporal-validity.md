The time interval in which a statement is considered applicable.

## The problem at this stage

A person can have one role last year and a different role today without either historical statement being meaningless. Time prevents yesterday’s truth from being silently presented as today’s context.
...
Validity intervals use [valid_from, valid_to): the start is included and the end excluded. This snapshot does not claim the later temporal remediation is verified.

Assertions and Decisions support temporal fields and optional as-of reads.
...
|---|---|---|
| Effective time | When does the statement apply? | Distinguishes historical and current validity |
| Recorded time | When was it stored? | A fact may be learned later |

<!-- CHECKS -->

### If a correction recorded today applies to yesterday, which times differ?

Distinguish effective validity from recording time. Reject invalid intervals or supersession and retain predecessor evidence and correction links.
