...
test('pointer can cross a short gap to the glossary learn link',async({page})=>{
 await page.goto('/#/en/archive/learn/start-here');
 ...
 await popup.locator('a').click();await expect(page).toHaveURL(/runtime/);
});
test('definition dismisses outside its corridor and after learning navigation',async({page})=>{
 await page.goto('/#/en/archive/learn/start-here');const term=page.locator('.map-user .learn-term');await term.locator('button').first().hover();await expect(term.locator('.learn-definition')).toBeVisible();
 ...
 await page.goto('/#/en/archive/learn/remember');const branch=page.locator('.concept-branches .learn-term').first();await branch.locator('button').first().click();await branch.locator('a').click();await expect(branch.locator('.learn-definition')).toHaveCount(0);await expect(page.locator('.integrated-guide')).toBeVisible();
});
test('an open glossary never covers its own trigger',async({page})=>{
 await page.goto('/#/en/archive/learn/remember');
 ...
 await term.locator('.learn-definition a').click();await expect(page.locator('.integrated-guide')).toBeVisible();
});
