...
for(const lang of ['ko-KR','en'])test(`M2 roadmap round trip in ${lang}`,async({page})=>{
 ...
 await page.locator('.atlas-drawer details').first().locator('summary').click();await page.locator('.atlas-drawer .learn-term>button').click();await expect(page.locator('.learn-definition')).toBeVisible();await page.keyboard.press('Escape');await expect(page.locator('.learn-definition')).toHaveCount(0);
